function mgs3(matrizValor,matrizConst,F,G,H,x0,y0,z0)%G É EM RELAÇÃO A Y E F É EM RELAÇÃO A X F é y isolado e G é x isolado
tol=1e-4;
...
for i = 1:1:40
    ...
    %Vantigo = Vatual;
    Vatual(1) = ((1/matrizValor(1,1))*(matrizConst(1)-matrizValor(1,2)*Vatual(2)-matrizValor(1,3)*Vatual(3)));
    Vatual(2) = ((1/matrizValor(2,2))*(matrizConst(2)-matrizValor(2,1)*Vatual(1)-matrizValor(2,3)*Vatual(3)));
    Vatual(3) = ((1/matrizValor(3,3))*(matrizConst(3)-matrizValor(3,1)*Vatual(1)-matrizValor(3,2)*Vatual(2)));
