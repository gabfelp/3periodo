function mgs(matrizValor,matrizConst,F,G,H,R,x0,y0,z0,w0)%G É EM RELAÇÃO A Y E F É EM RELAÇÃO A X F é y isolado e G é x isolado
tol=1e-4;
...
for i = 1:1:40
    ...
    %Vantigo = Vatual;
    Vatual(1) = ((1/matrizValor(1,1))*(matrizConst(1)-matrizValor(1,2)*Vatual(2)-matrizValor(1,3)*Vatual(3)-matrizValor(1,4)*Vatual(4)));
    Vatual(2) = ((1/matrizValor(2,2))*(matrizConst(2)-matrizValor(2,1)*Vatual(1)-matrizValor(2,3)*Vatual(3)-matrizValor(2,4)*Vatual(4)));
    Vatual(3) = ((1/matrizValor(3,3))*(matrizConst(3)-matrizValor(3,1)*Vatual(1)-matrizValor(3,2)*Vatual(2)-matrizValor(3,4)*Vatual(4)));
    Vatual(4) = ((1/matrizValor(4,4))*(matrizConst(4)-matrizValor(4,1)*Vatual(1)-matrizValor(4,2)*Vatual(2)-matrizValor(4,3)*Vatual(3)));
